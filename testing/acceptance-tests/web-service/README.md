# Testing Web Services

* [Requests Library](http-requests/README.md)

* Example: Article Service
    - Service: [article-service](article-service/)
    - Exercise: [article-service-test](article-service-test-exercise/)
    - [Model Solution](article-service-test/)

* Example: Book Service
    - Service: [book-service](book-service/)
    - Exercise: [book-service](book-service-test-exercise/)
    - [Model Solution](book-service-test/)

    - Demo: [book-service-test-json](book-service-test-json/)
    - Demo: [book-service-test-startserver](book-service-test-startserver/)

*Egon Teiniker, 2020-2026, GPL v3.0*